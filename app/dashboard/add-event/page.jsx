...
const AddEvent = () => {
  ...
  const handleAdd = (index, value) => {
    setEvent((prev) => ({ ...prev, rules: { ...prev.rules, [index]: value } }));
  };

  const handleClick = (e) => {
    e.preventDefault();
    const index = Object.values(event.rules).length;
    setEvent((prev) => ({
      ...prev,
...
export default isNotAuth(AddEvent);
